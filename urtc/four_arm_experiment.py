#!/usr/bin/env python3
"""
four_arm_experiment.py — controlled four-arm test for reviewer Major #10.
...
Run from the project root (needs OPENAI_API_KEY / the same env the main runner uses):
    ...
    python four_arm_experiment.py

Outputs one JSON per cell under results/four_arm/ plus a combined
results/four_arm_summary.csv.
...
# .four_arm_config.json (the GUI saves your entry there); and finally a hidden
...
SCCUR = ROOT / "SCCUR_TESTS"
sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(SCCUR))
...
OUT_DIR = ROOT / "results" / "four_arm"
OUT_DIR.mkdir(parents=True, exist_ok=True)

RUNNER_CONFIG = ROOT / ".permutation_runner_config.json"      # shared with the runner
FOURARM_CONFIG = ROOT / ".four_arm_config.json"               # this tool's own store


def _load_saved_key() -> str:
    for p in (FOURARM_CONFIG, RUNNER_CONFIG):
        try:
...
def _save_key(key: str) -> None:
    ...
    try:
        d = {}
        if FOURARM_CONFIG.exists():
            d = json.loads(FOURARM_CONFIG.read_text(encoding="utf-8"))
        d["openai_api_key"] = key
        FOURARM_CONFIG.write_text(json.dumps(d, indent=2), encoding="utf-8")
    except Exception:
        pass
...
class FourArmGenerator(InvestmentStrategyGenerator):
...
SUMMARY_CSV = ROOT / "results" / "four_arm_summary.csv"
...
def run_one_cell(api_key, cell, run_idx=0):
    ...
    gen = FourArmGenerator(api_key_openai=api_key, temperature=cell["temp"],
