    ...
    def _filter_financial_records_as_of(self, records, as_of_dt):
        ...
        for record in records or []:
            date_str = (
                ...
                or record.get("reportedDate")
                or record.get("acceptedDate")
                or record.get("date")
            )
            if not date_str:
                continue
            record_dt = pd.to_datetime(date_str, errors="coerce", utc=True)
            if pd.notna(record_dt):
                record_dt = record_dt.tz_convert(None)
            if pd.notna(record_dt) and record_dt <= as_of_dt:
